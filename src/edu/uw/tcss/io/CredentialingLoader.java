...
import static edu.uw.tcss.res.R.UsersFile.SALT;
import static edu.uw.tcss.res.R.UsersFile.HASH;
import static edu.uw.tcss.res.R.UsersFile.USERNAME;
import static edu.uw.tcss.res.R.UsersFile.CAMPUS;
        ...
        try (Scanner input = new Scanner(Paths.get(R.Strings.IO_FILE_LOCATION
            ...
            while (input.hasNextLine()) {
                ...
                if (!lineAsString.startsWith(R.Strings.IO_FILE_COMMENT)) {
                    ...
                    final String username = parts[USERNAME.ordinal()];
                    if (username.equals(theUsername)
                        && testHashForEquality(byteStringToByteArray(parts[SALT.ordinal()]),
                                               byteStringToByteArray(parts[HASH.ordinal()]),
                                                thePassword)) {
                        result = parts[CAMPUS.ordinal()];
                        break;
                    } 
                }
            }
            ...
            while (input.hasNextLine() && !found) {
                ...
                if (!lineAsString.startsWith(R.Strings.IO_FILE_COMMENT)) {
                    found = theUsername.equals(lineAsString.split(
                        R.Strings.IO_FILE_DELIMITER)[USERNAME.ordinal()]);
                }
            }
        } catch (final IOException e) {
            LOGGER.severe(e.toString());
